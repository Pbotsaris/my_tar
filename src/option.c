 ...
 *
 *
                - PRIVATE: select option. Returns ERROROPT if option is not
 found.
 ...
 *
 *
                - PRIVATE: searches for a -f flag. flag_opt to missing_f in case
 of failure
 ...
 *    - there is not default options. User must select one of the following
 * otheise an error will be returned
 *    *** [-c] [-u] [-t] [-x]
 *    - Options must be accompanied by [-f] otherwise program will return an
 * error.
 *    - ERROR: function will print error message but return error type to be
 ...
 *
 *    - looks for the starting position of the paths arguments in relation to
 * argv.
